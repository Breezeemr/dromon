...
- `fhir-store-datomic/docs/tasks/35-tenant-lifecycle.md` — Implements the three methods on `DatomicStore`, delegating to `ensure-tenant-conn!` and refactoring the existing `:fhir-store/datomic-store` halt-key to iterate via the new `delete-tenant`. **Lives in the fhir-store-datomic repo**, not this dromon repo.
...
- [fhir-store-tenant-lifecycle-protocol.md](fhir-store-tenant-lifecycle-protocol.md) — Added `create-tenant` / `delete-tenant` / `warmup-tenant` to `IFHIRStore` with docstrings covering `:if-exists`, `:if-absent`, `:close-storage?`, and `:resource-types` opts. The `test-server/seeder` now calls `create-tenant {:if-exists :ignore}` and `warmup-tenant` at boot before seeding SearchParameters, shifting `store/node.start` cold-start cost out of the first `http/request` and under the seeder's init span.
- [fhir-store-xtdb2-tenant-lifecycle.md](fhir-store-xtdb2-tenant-lifecycle.md) — Implemented the three methods on `XTDBStore` with `:store/create-tenant`, `:store/delete-tenant`, and `:store/warmup-tenant` spans. `delete-tenant` closes the XTDB node and, when `:close-storage? true` on a persistent backend, recursively deletes the `:log` / `:storage` `[:local {:path p}]` directories via a new private `delete-tenant-storage!` helper. Existing methods still go through `get-or-create-node` (Step A); removing that prelude is deferred to a follow-up. Nine new tests in `fhir-store-xtdb2/test/fhir_store_xtdb2/core_test.clj`: `clj -M:test` is 15 tests / 66 assertions / 0 failures on JDK 21.
- [fhir-store-mock-tenant-lifecycle.md](fhir-store-mock-tenant-lifecycle.md) — Implemented the three methods on `MockStore` as atom swaps. `create-tenant` does an atomic single-`swap!` that throws the 409 from inside the swap fn so concurrent losers see a no-op; `warmup-tenant` is a `(fnil identity {})` ensure. Three new `deftest`s in `fhir-store-mock/test/fhir_store/mock/core_test.clj` covering all opts branches: `clj -M:test` is 12 tests / 55 assertions / 0 failures. `test-server` (90/359) and `fhir-server` (30/158) suites both still pass.
