...
;; ---------------------------------------------------------------------------
;; collapse-thread: rewrite the sch body into a single mu/merge + static
;; [:map ...] literal, hoisted into a let-binding. This avoids the ~O(N^2)
;; per-call re-compilation that the chained mu/assoc / mu/update-properties /
;; mu/update-entry-properties / mu/optional-keys pipeline does today — at each
;; -into-schema invocation Malli currently rebuilds the map schema once per
;; entry. With this transform each -into-schema call is one m/schema on the
;; delta literal followed by one mu/merge with the base, plus any residual
;; ops that cannot be statically collapsed (mu/dissoc, mu/update, or updates
;; that target inherited-only keys). The delta literal itself is closed over
;; a let binding so it's allocated once per sch reify, not per call.
;;
;; Design rules (bail-on-unknown per spec):
;; - Collapsible: mu/update-properties with a literal merge or with the
;;   emitted (fn [props] (-> (or props {:closed true}) (assoc :resourceType "X")))
;;   shape; mu/assoc; mu/update-entry-properties merge on an own key;
;;   mu/optional-keys on all-own keys.
;; - Residual: mu/dissoc, mu/update, mu/update-entry-properties / mu/optional-keys
;;   on inherited-only keys, and any operation on a key a prior residual has
;;   already touched (ordering preservation).
;; - Anything else: throw ex-info so a regression surfaces at generation time.

(defn- -rt-resource-type
  "Recognise the emitted-by-splice-resource-type-form fn shape
   `(fn [props] (-> (or props {:closed true}) (assoc :resourceType \"X\")))`
   and return X, or nil if the form doesn't match."
  [f]
  (when (and (seq? f) (= 'fn (first f)))
    (let [s (pr-str f)]
      (when (re-find #"\(or props \{:closed true\}\)" s)
        (when-let [m (re-find #":resourceType \"([^\"]+)\"" s)]
          (second m))))))

(defn- -form-contains-symbol?
  "Tree-walk `form`, return true if `sym` appears anywhere as a value."
  [form sym]
  (let [found? (volatile! false)]
    (clojure.walk/postwalk
      (fn [x] (when (and (not @found?) (= x sym)) (vreset! found? true)) x)
      form)
    @found?))

(defn- -options-dependent?
  "The collapse runs inside a let binding where `options` (the -into-schema
   method parameter) is not yet in scope. Any sub-form that references
   `options` has to stay inside the reify body — mark it residual."
  [form]
  (-form-contains-symbol? form 'options))

(defn- -classify-step
  "Classify a thread step for the collapse walker. Returns a map with
   :kind and the relevant payload, or nil if the step is unknown."
  [step]
  (cond
    (= step 'm/deref) {:kind :setup :step step}

    (and (seq? step)
         (or (= 'm/schema (first step))
             (= 'm/deref  (first step))))
    {:kind :setup :step step}

    (and (seq? step) (symbol? (first step))
         (= "mu" (namespace (first step))))
    (let [op (name (first step))
          args (rest step)]
      (case op
        "update-properties"
        (let [[f arg] args]
          (cond
            (and (or (= 'merge f) (= 'clojure.core/merge f))
                 (map? arg))
            {:kind :map-props :props arg}

            (nil? arg)
            (when-let [rt (-rt-resource-type f)]
              {:kind :map-props :props {:closed true :resourceType rt}})

            :else nil))

        "assoc"
        (when (= 2 (count args))
          (if (-options-dependent? (second args))
            ;; The value references `options` — has to run inside the reify
            ;; body. Route through residuals, but keep the key so ordering is
            ;; tracked correctly.
            {:kind :residual :step step}
            {:kind :assoc :key (first args) :value (second args)}))

        "update-entry-properties"
        (when (= 3 (count args))
          (let [[k f props] args]
            (when (and (or (= 'merge f) (= 'clojure.core/merge f))
                       (map? props)
                       (not (-options-dependent? props)))
              {:kind :update-entry-props :key k :props props :step step})))

        "optional-keys"
        (when (and (= 1 (count args)) (vector? (first args)))
          {:kind :optional-keys :keys (first args) :step step})

        "dissoc"   {:kind :residual :step step}
        "update"   {:kind :residual :step step}
        nil))
    :else nil))

(defn- -empty-collapse-state []
  {;; :setup → haven't seen any mutation yet (setup steps allowed)
   ;; :delta → inside the contiguous run of collapsible mu-ops (setup ops
   ;;          seen here are a hard stop — they would otherwise reorder)
   ;; :trailing → everything from here on stays in-order as thread steps
   :phase :setup
   :setup []
   :map-props {}
   :entry-order []
...
(defn- -go-trailing [state step]
  (-> state
      (assoc :phase :trailing)
      (update :trailing conj step)))

(defn- -apply-collapse-step [state step]
  (let [c (-classify-step step)]
    (when (nil? c)
      (throw (ex-info "collapse-thread: unrecognised step"
                      {:step step})))
    (case (:phase state)
      :trailing
      ;; Once trailing, preserve every subsequent step verbatim in order.
      (update state :trailing conj step)

      :setup
      (case (:kind c)
        :setup     (update state :setup conj (:step c))
        :map-props (-> state (assoc :phase :delta) (update :map-props merge (:props c)))
        :assoc     (let [k (:key c) v (:value c)]
                     (-> state
                         (assoc :phase :delta)
                         (update :entry-order conj k)
                         (assoc-in [:entries k] {:value v :props nil :optional? false})
                         (update :own-keys conj k)))
        :update-entry-props (-go-trailing state step)
        :optional-keys      (-go-trailing state step)
        :residual           (-go-trailing state step))

      :delta
      (case (:kind c)
        ;; Any non-mu op mid-delta is a hard stop — setup-class steps like
        ;; m/deref that appear after the first mutation can't be safely
        ;; reordered across mu/merge, so we flush into trailing and stop.
        :setup (-go-trailing state step)

        :map-props
        (update state :map-props merge (:props c))

        :assoc
        (let [k (:key c) v (:value c)]
          (-> state
              (cond-> (not (contains? (:entries state) k))
                (update :entry-order conj k))
              (assoc-in [:entries k] {:value v :props nil :optional? false})
              (update :own-keys conj k)))

        :update-entry-props
        (let [k (:key c) props (:props c)]
          (if (contains? (:own-keys state) k)
            (update-in state [:entries k :props] (fnil merge {}) props)
            (-go-trailing state step)))

        :optional-keys
        (let [ks (:keys c)]
          (if (every? #(contains? (:own-keys state) %) ks)
            (reduce (fn [s k] (assoc-in s [:entries k :optional?] true)) state ks)
            (-go-trailing state step)))

        :residual
        (-go-trailing state step)))))

(defn- -emit-delta-form [{:keys [map-props entry-order entries]}]
  (let [entry-vectors
        (mapv (fn [k]
                (let [{:keys [value props optional?]} (get entries k)
                      merged-props (cond-> (or props {})
                                     optional? (assoc :optional true))]
                  (if (seq merged-props)
                    [k merged-props value]
                    [k value])))
              entry-order)]
    (into [:map (if (seq map-props) map-props {})] entry-vectors)))

(defn- -head-is-base-map? [head]
  (and (seq? head) (= 'm/schema (first head)) (= :map (second head))))

(defn collapse-thread
  "Rewrite an `->`-threaded sch body into a (let [delta-form <literal>] <body>)
   form where <body> is a thread that uses a single mu/merge on the literal.
   See the top-of-section block comment for semantics. Returns the form
   unchanged when it doesn't look like a thread form at all; throws
   ex-info on any unrecognised step (per design)."
  [thread-form]
  (if-not (and (seq? thread-form) (= '-> (first thread-form)))
    thread-form
    (let [[_ head & steps] thread-form
          base-map? (-head-is-base-map? head)
          ;; For base-inheriting files, mu/merge can't preserve the base's
          ;; per-entry `:optional true` the way the original mu/assoc
          ;; chain does (mu/merge re-computes `:optional` from the
          ;; right-side's entry properties, and nil-props reads as
          ;; required). Collapse only the schema-level props in that
          ;; case, letting the original entry ops stay in the thread via
          ;; the trailing list. `:map`-rooted files have no base to
          ;; preserve from, so we can collapse entries there safely.
          final (reduce -apply-collapse-step (-empty-collapse-state) steps)
          ;; If this isn't a :map-head and we have entries, demote them
          ;; into the trailing list in their original order before the
          ;; trailing residuals that were already collected.
          final (if (and (not base-map?) (seq (:entry-order final)))
                  (let [entry-ops
                        (mapcat
                          (fn [k]
                            (let [{:keys [value props optional?]} (get (:entries final) k)
                                  ;; Reconstruct the original op shape.
                                  ;; Everything in :entries was added via
                                  ;; :assoc, possibly with one or more
                                  ;; subsequent :update-entry-props merges
                                  ;; and an optional flag.
                                  assoc-step `(~'mu/assoc ~k ~value)
                                  uep-step   (when (seq props)
                                               `(~'mu/update-entry-properties
                                                  ~k ~'merge ~props))
                                  opt-step   (when optional?
                                               `(~'mu/optional-keys [~k]))]
                              (filterv some? [assoc-step uep-step opt-step])))
                          (:entry-order final))]
                    (-> final
                        (assoc :entry-order [] :entries {} :own-keys #{})
                        (update :trailing #(into (vec entry-ops) %))))
                  final)
          delta-form (-emit-delta-form final)
          {:keys [setup trailing]} final
          has-delta? (or (seq (:entry-order final)) (seq (:map-props final)))
          body (cond
                 base-map?
                 (apply list '-> `(~'m/schema ~'delta-form ~'options) trailing)

                 (not has-delta?)
                 (apply list '-> head (concat setup trailing))

                 :else
                 (apply list '-> head
                        (concat setup
                                ;; 3-arg mu/merge so the caller's options
                                ;; (including any :ref registry override)
                                ;; propagate into the merged schema. The
                                ;; 2-arg form defaults options to nil and
                                ;; drops the lazy-ref override, which breaks
                                ;; later mu/* calls on the returned schema.
                                [`(~'mu/merge (~'m/schema ~'delta-form ~'options) ~'options)]
                                trailing)))]
      (if has-delta?
        `(~'let [~'delta-form ~delta-form] ~body)
        body))))

(defn make-into-schema-form [form]
  (let [form      (dedup-forms form)
        collapsed (collapse-thread form)]
    (if (and (seq? collapsed) (= 'let (first collapsed)))
      (let [[_ bindings body] collapsed]
        `(~'let ~bindings
                (~'reify ~'m/IntoSchema
                         (~'-into-schema [~'_ ~'_ ~'_ ~'options] ~body))))
      `(~'reify ~'m/IntoSchema
                (~'-into-schema [~'_ ~'_ ~'_ ~'options] ~collapsed)))))
